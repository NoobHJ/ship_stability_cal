...
async def calculate_waterplane_area(data, result_list):
    for i in range(len(data.index)):
        ...
        for j in range(2, len(data.columns), 2):
            a.append(await simpson(data.iloc[i,j-2], 
                            data.iloc[i,j-1], 
                            data.iloc[i,j]))
...
async def calculate_volume(data, result_list):    
    if len(data) % 2 == 0:
        for i in range(2, len(data), 2):
            result_list.append(await simpson(data[i-2],
                                data[i-1],
                                data[i],))
    else:
        for i in range(2, len(data)-2, 2):
            result_list.append(await simpson(data.iloc[i-2],
                                       data.iloc[i-1],
                                       data.iloc[i],))
        result_list.append(await simpson2(data.iloc[-4],
                            data.iloc[-3],
                            data.iloc[-2],
                            data.iloc[-1]))
    return result_list
...
async def calculate_section_area(data, result_list):
    for j in range(len(data.columns)):
        ...
        for i in range(2, len(data.index), 2):
            a.append(await simpson(data.iloc[i-2, j], 
                            data.iloc[i-1, j], 
                            data.iloc[i, j]))
        result_list.append(a)
    return result_list
...
async def calculate_moment_of_inertia(data, result_list, masscenter_x):
    for j in range(len(data.columns)):
        ...
        for i in range(2, len(data.index), 2):
            ...
            a.append(await second_moment_simpson(data.iloc[i-2, j], 
                                           data.iloc[i-1, j], 
                                           data.iloc[i, j],
                                           s1, s2, s3))
...
async def cal_run(offset_path):
    ...
    for i in range(len(offset.index)):
        a = []
        for j in range(2, len(offset.columns), 2):
            a.append(await simpson(offset.iloc[i,j-2], 
                            offset.iloc[i,j-1], 
                            offset.iloc[i,j]))
    ...
    for i in range(2, len(first_moment_z), 2):
        ...
        s2 = 0.5 + i - 1
        s3 = 0.5 + i
        total_momentz.append(await moment_simpson(first_moment_z[i-2], 
                                            first_moment_z[i-1], 
                                            first_moment_z[i], 
                                            s1, s2, s3))
